    ...
    for epoch in range(EPOCH):
        ...
        for step, (b_x, b_y) in enumerate(train_loader):
            ...
            if step % TEST_NUM == 0:
                ...
                man_file1 = open(RESULT_ROOT + '[' + str(EDGE_NAME) + ']' + '[NonDP-Accuracy]' + '.txt', 'a')
                accuracy = correct / total
                man_file1.write(str(accuracy) + "\n")
                man_file1.close()

                man_file2 = open(RESULT_ROOT + '[' + str(EDGE_NAME) + ']' + '[NonDP-TestLoss]' + '.txt', 'a')
                test_loss = test_loss / (i + 1)
                man_file2.write(str(test_loss) + "\n")
                man_file2.close()

                man_file3 = open(RESULT_ROOT + '[' + str(EDGE_NAME) + ']' + '[NonDP-TrainLoss]' + '.txt', 'a')
