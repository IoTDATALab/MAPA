...
def noise(grads,z,Clipbound):
    [w_c1, b_c1, w_c2, b_c2, w_o, b_o] = grads
    size = 10 * 1 * 5 * 5 + 10 + 10 * 10 * 5 * 5 + 10 + 62 * 160 + 62
    noise_std = np.random.normal(loc = 0, scale = z*Clipbound, size = size) #scale
    w_c1_noise_std = torch.from_numpy(noise_std[0:250].reshape(10, 1, 5, 5)).float()
    w_c1 = w_c1 + w_c1_noise_std
    b_c1_noise_std = torch.from_numpy(noise_std[250:250 + 10].reshape(10)).float()
    b_c1 = b_c1 + b_c1_noise_std
    w_c2_noise_std = torch.from_numpy(noise_std[250 + 10:250 + 10 + 2500].reshape(10, 10, 5, 5)).float()
    w_c2 = w_c2 + w_c2_noise_std
    b_c2_noise_std = torch.from_numpy(noise_std[250 + 10 + 2500:250 + 10 + 2500 + 10].reshape(10)).float()
    b_c2 = b_c2 + b_c2_noise_std
    w_o_noise_std = torch.from_numpy(noise_std[250 + 10 + 2500 + 10:250 + 10 + 2500 + 10 + 160*62].reshape(62, 160)).float()
    w_o = w_o + w_o_noise_std
    b_o_noise_std = torch.from_numpy(noise_std[250 + 10 + 2500 + 10 + 160*62:250 + 10 + 2500 + 10 + 160*62 + 62].reshape(62)).float()
    b_o = b_o + b_o_noise_std
    grads_noise = [w_c1, b_c1, w_c2, b_c2, w_o, b_o]
    return grads_noise
...
def Addnoise(Tensor, Clipboud):
    if len(Clipboud) == 1:
        for i in range(len(Tensor)):
            Tensor[i] = Tensor[i] + z * Clipboud[0] * torch.randn(Tensor[i].shape)
    if len(Clipboud) > 1:
        for i in range(len(Tensor)):
            Tensor[i] = Tensor[i] + z * Clipboud[i] * torch.randn(Tensor[i].shape)
    return Tensor
...
if __name__=='__main__':
    ...
    for epoch in range(EPOCH):
        ...
        for step, (b_x, b_y) in enumerate(train_loader):
            ...
            loss_batch=0
            grads_batch=zerosgrad(Layers_shape)
            for idx in range(b_x.size(0)):
                b_xx = torch.unsqueeze(b_x[idx], 0)
                b_yy = torch.unsqueeze(b_y[idx], 0)
                output = cnn.forward(b_xx)[0]
                loss = nn.CrossEntropyLoss()(output, b_yy)
                cnn.zero_grad()
                loss.backward()
                grads_user=[]
                for params in cnn.parameters():
                    grads_user.append(params.grad.data)
                ClipBound = Clipb(G, Layers_nodes, style="Flat")
                grads_user=Clip(grads_user, ClipBound)[0]
                loss_batch += loss.item()
                for i in range(Layers_num):
                    grads_batch[i]+=grads_user[i]
            for i in range(Layers_num):
                grads_batch[i]=grads_batch[i]/(idx+1)
            grads_noise = Addnoise(grads_batch,ClipBound/(idx+1))
            loss_batch=loss_batch/(idx+1)
            ...
            client.publish("mapa_grads/" + CLIENT_ID, cPickle.dumps(grads_noise), 2)  #
            p = cPickle.loads(msgQueue.get())
            z = p[0]
            G = p[1]
            params = p[2]
