...
client.subscribe([("init",2), ("ada2_params/"+CLIENT_ID, 2)])
client.loop_start()
    ...
    for epoch in range(EPOCH):
        ...
        for step, (b_x, b_y) in enumerate(train_loader):
            ...
            if step % TEST_NUM == 0:
                ...
                man_file1 = open(RESULT_ROOT + '[' + str(EDGE_NAME) + ']' + '[Adaclip2-Accuracy]', 'w')
                accuracy = correct / total
                acc.append(accuracy)
                print(acc, file=man_file1)
                man_file1.close()

                man_file2 = open(RESULT_ROOT + '[' + str(EDGE_NAME) + ']' + '[Adaclip2-TestLoss]', 'w')
                test_loss = test_loss / (i + 1)
                Losstest.append(test_loss)
                print(Losstest, file=man_file2)
                man_file2.close()

                man_file3 = open(RESULT_ROOT + '[' + str(EDGE_NAME) + ']' + '[Adaclip2-TrainLoss]', 'w')
            ...
            grads_=[w_c1, b_c1, w_c2, b_c2, w_o, b_o]
            grads = [-LR*w_c1,-LR*b_c1, -LR*w_c2, -LR*b_c2, -LR*w_o, -LR*b_o]
            delta ,indicator= Clip(grads, Clipbound)
            payload =[delta ,indicator]
            client.publish("ada2_grads/"+CLIENT_ID, cPickle.dumps(payload), 2)
