...
# vis = Visdom(env='SYNTHETIC_NonDP_ASyn')
...
print(len(Federate_Dataset))

######## Initial model accuracy on test dataset #######
test_loss, test_acc = test(model, device, test_loader)

# Logging files for experiment results
with open('../results/SYNTHETIC/NonDP_Asyn_TestLoss_flat.txt', 'a+') as fl:
...
with open('../results/SYNTHETIC/NonDP_Asyn_TestAcc_flat.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {}, CS is {}, LF is {})'.
             format(date, Users_num_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound,
                    args.ClipStyle, args.Leaf))
    fl.write(str(test_acc) + '\t')
with open('../results/SYNTHETIC/NonDP_Asyn_TrainLoss_flat.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {}, CS is {}, LF is {})'.
             format(date, Users_num_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound,
                    args.ClipStyle, args.Leaf))
...
for itr in range(1, args.itr_numbers + 1):
    ...
    for idx_outer, (train_data, train_targets) in enumerate(federated_train_loader):
        ...
        # Clip
        Gradients_Tensor, _ = TensorClip(Gradients_Tensor, Clip_Bound)
        Loss_train += loss
    ...
    for i in range(Layers_num):
        ...
        # print(Collect_Gradients[i].norm())
    # Average loss
    Loss_train /= (idx_outer + 1)

    # Update the global model using conventional gradient descent
    for idx_par, params_sever in enumerate(model.parameters()):
        params_sever.data.add_(-lr, Collect_Gradients[idx_par])
