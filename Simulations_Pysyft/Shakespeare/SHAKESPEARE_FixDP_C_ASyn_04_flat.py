...
vis = Visdom(env='Shakes_FixDP_C_ASyn_04flat')

# Define parameters
class Arguments():
    def __init__(self):
        self.batch_size = 10 # Number of samples used of each user/device at each iteration.
        # If this value is less than 1, then it means the sampling ratio, else it means the mini-batch size
        self.lr = 0.2  # Learning rate
        self.grad_upper_bound = torch.tensor([0.1])  # clipbound
...
######## Logging files #######
with open('../results/SHAKESPEARE/FixDP_C_Asyn_04flat_TestLoss.txt', 'a+') as fl:
...
with open('../results/SHAKESPEARE/FixDP_C_Asyn_04flat_TestAcc.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound))
    fl.write(str(test_acc) + '\t')
with open('../results/SHAKESPEARE/FixDP_C_Asyn_04flat_TrainLoss.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound))
with open('../results/SHAKESPEARE/FixDP_C_Asyn_04flat_Budget.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound))
...
delta = 1. / args.users_total**(1.1) # delta in (varepsilon, delta)-DP
...
for itr in range(1, args.itr_numbers + 1):
    ...
    for idx_outer, (train_data, train_targets) in enumerate(federated_train_loader):
        ...
        Gradients_user, Loss_user = train(args.lr, model_round, train_data, train_targets, device, optimizer,
                                          gradient=True)
