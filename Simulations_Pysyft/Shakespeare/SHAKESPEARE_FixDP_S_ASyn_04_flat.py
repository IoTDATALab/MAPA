...
vis = Visdom(env='Shakes_FixDP_ASyn_S_04flat')
...
class Arguments():
    def __init__(self):
        ...
        self.grad_upper_bound = torch.tensor([0.01])  # clipbound
...
class Net(nn.Module):
    ...
    def forward(self, input):
        ...
        output, _ = self.lstm(decoder)
        output = self.linear(output[:, -1, :])
        # output = self.linear(output.contiguous().view(output.shape[0], -1))
        return output

    # def init_hidden(self):
    #     return Variable(torch.zeros(self.num_layer, args.batch_size, self.hidden_size))
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            output = model(data)
            test_loss +=  (Criteria(output, target) * data.shape[0]).item() # sum up batch loss
...
######## logging files #######
with open('../results/SHAKESPEARE/FixDP_Asyn_04flat_TestLoss.txt', 'a+') as fl:
...
with open('../results/SHAKESPEARE/FixDP_Asyn_04flat_TestAcc.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound))
    fl.write(str(test_acc) + '\t')
with open('../results/SHAKESPEARE/FixDP_Asyn_04flat_TrainLoss.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound))
with open('../results/SHAKESPEARE/FixDP_Asyn_04flat_Budget.txt', 'a+') as fl:
    fl.write('\n {} Results \n (UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, args.batch_size, args.lr, args.z, args.grad_upper_bound))
...
lr = args.lr
# Set clipping bound
Clip_Bound = ClipBound_gerate(args.grad_upper_bound, Layers_nodes, style="Flat")
...
for itr in range(1, args.itr_numbers + 1):
    ...
    for idx_outer, (train_data, train_targets) in enumerate(federated_train_loader):
        ...
        # Average all noisy gradients of the current users/participants
        for i in range(Layers_num):
            Collect_Gradients[i] += Collect_Gradients_Per_user[i]
    # Update the global model using the averaged gradient
    for grad_idx, params_server in enumerate(model.parameters()):
        params_server.data.add_(-lr, Collect_Gradients[grad_idx] / (idx_outer + 1))
        # params_server.data = params_server - lr * Collect_Gradients[grad_idx]
    # Average total loss of the current users/participants
