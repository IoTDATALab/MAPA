...
vis = Visdom(env='CELEBA_MAPA_S_Asyn_08_flat')

#定义参量
class Arguments():
    def __init__(self):
        self.batch_size = 0.2 # Number of samples used of each user/device at each iteration.
        ...
        self.L_smooth = 0.00007  # L smooth constant of gradient
        ...
        self.beta_1 = 0.99  # Averaging speed parameter for the mean of gradients, only used for AdaClip1
        self.beta_2 = 0.9  # Averaging speed parameter for the variance of gradients, only used for AdaClip1
        self.h_1 = torch.tensor(10 ** (-12))  # Parameter for estimating stand variance, only used for AdaClip1
        self.h_2 = torch.tensor(10 ** (-10))  # Parameter for estimating stand variance, only used for AdaClip1
        self.c = torch.tensor([0.5])  # Split ratio of privacy budget only used for AdaClip2
        self.quatile = 0.5   # Target unclipped gradient, only used for AdaClip2
...
with open('../results/CELEBA/MAPA_S_Asyn_08flat_TestLoss.txt', 'a+') as fl:
...
with open('../results/CELEBA/MAPA_S_Asyn_08flat_TestAcc.txt', 'a+') as fl:
    fl.write('\n ' + date + ' Results (Total users number is ' + str(args.users_total))
    fl.write(', BZ is {}, z is {}, {}, CB is {}'.format(args.batch_size, args.z, args.ClipStyle, args.grad_upper_bound))
    fl.write(', L is {}, inital bound is {} \n'.format(args.L_smooth, args.inital_bound))
with open('../results/CELEBA/MAPA_S_Asyn_08flat_Budget.txt', 'a+') as fl:
    fl.write('\n ' + date + ' Results (Total users number is ' + str(args.users_total))
    fl.write(', BZ is {}, z is {}, {}, CB is {}'.format(args.batch_size, args.z, args.ClipStyle, args.grad_upper_bound))
    fl.write(', L is {}, inital bound is {} \n'.format(args.L_smooth, args.inital_bound))
with open('../results/CELEBA/MAPA_S_Asyn_08flat_TrainLoss.txt', 'a+') as fl:
    fl.write('\n ' + date + ' Results (Total users number is ' + str(args.users_total))
    fl.write(', BZ is {}, z is {}, {}, CB is {}'.format(args.batch_size, args.z, args.ClipStyle, args.grad_upper_bound))
    fl.write(', L is {}, inital bound is {} \n'.format(args.L_smooth, args.inital_bound))
