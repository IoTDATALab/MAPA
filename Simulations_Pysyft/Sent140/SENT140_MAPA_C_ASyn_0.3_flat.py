...
vis = Visdom(env='SENT140_MAPA_ASyn')
...
class Arguments():
    def __init__(self):
        ...
        self.L_smooth = 2 * 10 ** (-5)  # L smooth constant of gradient
        self.grad_var = 5  # Variance of gradients on samples
        self.grad_upper_bound = torch.tensor([1.])  # clipbound
        self.redu_ratio = torch.tensor(0.5)  # Reductio ratio
...
with open('../results/SENT140/MAPA_Asyn_TestLoss.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, L_smooth is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.L_smooth, args.z, args.grad_upper_bound))
    fl.write(str(test_loss.item()) + '\t')
with open('../results/SENT140/MAPA_Asyn_TestAcc.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, L_smooth is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.L_smooth, args.z, args.grad_upper_bound))
    fl.write(str(test_acc.item()) + '\t')
with open('../results/SENT140/MAPA_Asyn_TrainLoss.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, L_smooth is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.L_smooth, args.z, args.grad_upper_bound))
with open('../results/SENT140/MAPA_Asyn_Budget.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, L_smooth is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.L_smooth, args.z, args.grad_upper_bound))
...
for itr in range(1, args.itr_numbers + 1):
    ...
    for idx_outer, (train_data, train_targets) in enumerate(federated_train_loader):
        ...
        train_targets, length = torch.split(train_targets, 1, dim=1)

        train_data = torch.nn.utils.rnn.pack_padded_sequence(train_data, length[0], batch_first=True)

        Gradients_user, Loss_user = train(Stage_Lr, model_round, train_data, train_targets[0],
                                              gradient=True)
    ...
    if itr % args.log_test == 0:
        ...
        print(Sampled_ratio,args.z, itr, delta)
