...
vis = Visdom(env='SENT140_AdaClip1_ASyn')
...
class Arguments():
    def __init__(self):
        ...
        self.ClipBound = torch.tensor(10)  # clipbound
...
def UpdateMS(Gradients, M, S, B, args, Layers_num,Clip_Bound):
    if len(Clip_Bound) == 1:
        for i in range(Layers_num):
            ...
            temp = torch.min(torch.max(((Gradients[i] - M[i]) ** 2 - B[i] ** 2 * (args.z*Clip_Bound) ** 2), args.h_1.float()),
                             args.h_2.float())
...
def Noise_Addition(Layers_num, Layers_shape, Gradients, M, B, ClipBound):
    ...
    if len(ClipBound) == 1:
        ...
        for i in range(Layers_num):
            ...
            Gradients[i] = Gradients[i] + 0.5*variance * ClipBound * torch.randn(Layers_shape[i])
            Gradients[i] = Gradients[i] * B[i] + M[i]
    elif len(ClipBound) > 1:
        for i in range(Layers_num):
            Gradients[i] = (Gradients[i] - M[i]) / B[i]
            norm = Gradients[i].float().norm()
            if ClipBound[i] < norm:
                Gradients[i] = ClipBound[i] * Gradients[i] / norm
            Gradients[i] = Gradients[i] + 0.5*variance * ClipBound[i] * torch.randn(Layers_shape[i])
            Gradients[i] = Gradients[i] * B[i] + M[i]
    return Gradients
...
with open('../results/SENT140/AdaClip1_Asyn_TestLoss.txt', 'a+') as fl:
...
with open('../results/SENT140/AdaClip1_Asyn_TestAcc.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.lr, args.z, args.ClipBound))
    fl.write(str(test_acc) + '\t')
with open('../results/SENT140/AdaClip1_Asyn_TrainLoss.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.lr, args.z, args.ClipBound))
with open('../results/SENT140/AdaClip1_Asyn_Budget.txt', 'a+') as fl:
    fl.write('\n {} Results \n (Flat, UN is {}, SP is {}, BZ is {}, LR is {}, SNR is {}, CB is {})'.
             format(date, args.users_total, args.user_sel_prob, batch_size, args.lr, args.z, args.ClipBound))
...
Sampled_num = batch_size * args.batchs_round
Sampled_ratio = Sampled_num / sum(Leaf_split)
...
for itr in range(1, args.itr_numbers + 1):
    ...
    for idx_outer, (train_data, train_targets) in enumerate(federated_train_loader):
        ...
        Gradients_Tensor, loss = train(lr, model_round, train_data, train_targets, idx_unsort, gradient=True)
