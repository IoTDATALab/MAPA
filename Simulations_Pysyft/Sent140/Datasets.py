...
class reddit(Dataset):
    ...
    def build_vocab(self,counter, vocab_size=1000):
        count_pairs = sorted(counter.items(), key=lambda x: (-x[1], x[0]))#先按照统计的个数counter[1]进行降序排列，相同的按照单词counter[0]来排
        count_pairs = count_pairs[:(vocab_size - 2)]#为<PAD>和<UNK>空出两个位置

        words, counters = list(zip(*count_pairs))#得到单词表，其中单词的顺序是按照单词出现频率来排的
...
def dataset_federate_noniid(dataset, workers,Ratio = [1, 1, 1], net='NOT CNN' ):
    ...
    for n in range(0, len(workers)):
        ...
        Subset = dataset_list[N:N+Ratio[n]]#抽取数据
